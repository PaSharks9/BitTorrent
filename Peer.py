...
class peerWorker(threading.Thread):
    ...
    def run(self):
        ...
        if(file_md5 not in sharedDict):
            lockSharedDict.release()
            print("[PEER_WORKER] WARNING: Requested file with MD5=" + str(file_md5) + " but i've not found inside SharedDict. Abort.")
            self.cSocket.close()
            return
        ...
        if(int(part_id) not in range(0, file_nParts)):
            print("[PEER_WORKER] WARNING: Requested part=" + str(part_id) + " but that file has " + str(file_nParts) + ". Abort.")
            lockSharedDict.release()
            self.cSocket.close()
            return
        ...
        if(file_mask[int(part_id)] == '0'):
            print("[PEER_WORKER] ERROR: Requested the part=" + str(part_id) + " of md5=" + str(file_md5) + " but i don't have that part. Abort.")
            lockSharedDict.release()
            self.cSocket.close()
            return
            ...
            if(self.working == True):    # se ho del lavoro da svolgere
                while True: 
                    ...
                    if sockDownload is None:
                        print("[POOL_WORKER_" + str(self.id) + "] ERROR: Cannot connect to the peer", self.peer[0], ":", self.peer[1], ". Retry...")
                        getch()
                    else:
                        break
...
def checkStatus(sid, md5, n_parts, sock, sLock):
    ...
    for i in range(0,hitpeer):
        ...
        for i in range(0, n_parts):
            ...
            if(maschera[i] is True):    mask.append('1')
            else:                       mask.append('0')
    ...
    while True:
        if(len(partsList) == n_parts):  # posso procedere solo dopo che l'updater ha processato almeno una volta la partsList
            ...
            try:
                ...
                if(debug is True):  print("[PEER] Random choosed part " + str(part_id) + " available from " + str(value) + " peers.")
                ...
                if(debug is True):  print("[PEER] Random choosed to require the part " + str(part_id) + " to the peer", peer_details)
...
def loadConfiguration(): 
    ...
    config = {}
    if len(from_csv) != 8:
        print("ERROR: Wrong number of parameters inside the configuration file (" + str(len(from_csv)) + "instead of 8).")
        return None
    else:
...
def logout(sock, Session_ID, sLock, peerProxy):
    ...
    if(data[0:4] == "ALOG"):
        ...
        last_value = -1
        while(getSending()>0):
            if(last_value != getSending()):  # in questo modo stampo la riga seguente solo quando il numero di invii cambia
...
def partModify(md5, partId, inc):
    ...
    if(part_id not in range(0,len(locked))):
        print("ERROR: partModify(" + str(md5) + ',' + str(part_id) + ',' + str(inc) + ") but there are only " + str(len(locked)) + " parts.")
        lockUseDict.release()
        return False

    if(locked[part_id] == -1):
        print("ERROR: partModify(" + str(md5) + ',' + str(part_id) + ',' + str(inc) + ") but i don't have this part (-1).")
        lockUseDict.release()
        return False

    if((inc != 1) and (inc != -1)):
        print("ERROR: partModify(" + str(md5) + ',' + str(part_id) + ',' + str(inc) + ") but the inc_value must be '1' or '-1'.")
        lockUseDict.release()
        return False
...
def partUpdate(md5):
    ...
    if(len(locked) != len(shared)):
        print("ERROR: partUpdate(" + str(md5) + ") but len(locked)=" + str(len(locked)) + " NOT EQUAL TO len(shared)=" + str(len(shared)) + ". Abort.")
        lockUseDict.release()
        return False
...
def partLocked(md5, part_id):
    ...
    if(part_id not in range(0,len(locked))):
        print("ERROR: partLocked(" + str(md5) + ',' + str(part_id) + ") but there are only" + str(len(locked)) + " parts.")
        return -1
...
def searchFile(sock, Session_ID, sLock, research):
    ...
    for i in range(0, idm5):
        data = recvExact(sock,148).decode('utf-8')  # ricevo {File_md5_i[32B].File_name_i[100B].len_file[10B].len_part[6B]}
...
def setCompleted(md5, part_id): # restituisce False se qualcosa è andato storto, True se ha completato con successo
    ...
    if(len(maschera) < part_id):
        print("ERROR: setCompleted called with part_id=" + str(part_id) + " but len(mask)=" + str(len(maschera)) + ". Abort.")
        lockSharedDict.release()
        return False

    if(maschera[part_id] == '1'):
        print("WARNING: setCompleted called with part_id=" + str(part_id) + " but it's already '1' inside the mask. Abort.")
        lockSharedDict.release()
        return False
    ...
    if(md5 in inUseDict):
        ...
        if(lista[part_id] == -1):
            lista[part_id] = 0
            inUseDict[md5] = lista
        else:                       print("WARNING: setCompleted expected a '-1' value inside inUseDict, but ", lista[part_id], " found!")
    lockUseDict.release()
